...
angularApp.controller('workflowCtrl', function($scope, $http,Restangular, $location, $compile, $stateParams, $route, $routeParams, $modal, flash) {
	var workflows = Restangular.all('workflows');	
	var counter = 2;
	...
	$scope.getWorkflowList = function(q) {
		...
		// var promise = workflowService.list({
		// 	suggestionFor: 'workflow',
		// 	q: q,
		// 	pageLimit: 20,
		// 	page: 1
		// });
	...
	$scope.getTotalWorkflow = function() {
		// var promise = workflowService.totalWorkflow({
		// 	countFor: 'workflow'
		// });
		// promise
		workflows.getList({countFor: 'workflow'}).then(function(result) {
	...
	$scope.selectWorkflow = function(value) {
		...
		// promise
		Restangular.one('workflows', value).get().then(function(result) {
				$scope.workflowDefinitions = [];
	...
	$scope.delete = function() {
		...
		// promise.then(function(msg) {
			Restangular.one('workflows',$routeParams.workflowId).remove().then(function(result) {
				$scope.getWorkflowList($scope.search);
				flash.pop({
					title: 'Success',
					body: msg,
	...
	$scope.create = function(workflow) {
		...
		// 			title: 'Success',
		// 			body: msg.msg,
		// 			type: 'success'workflows
		// 		});
		// 		$scope.workflowCreateLoader = false;
		// 		$scope.selectWorkflow(msg.id);
		...
		// 	}
		// );
		workflows.post(workflow).then(function(result) {
			$scope.workflowCreateLoader = false;
			flash.pop({
					title: 'Success',
					body: msg,
			...
			$window.location.href = "http://" + result.msg + "." + $rootScope.host + "/#/main/landing";
	...
	$scope.updateWorkflow = function() {
		...
		// promise
		$scope.workflow.put().then(function(msg) {
				$scope.workflowUpdateLoader = false;
				$scope.getWorkflowList($scope.search);
				flash.pop({
					title: 'Success',
	...
	$scope.deleteWorkflow = function() {
		...
		// var promise = workflowService.delete($routeParams.workflowId);
		// promise
		$scope.workflow.remove().then(function(msg) {
				$scope.getWorkflowList($scope.search);
				flash.pop({
					title: 'Success',
					body: msg,
