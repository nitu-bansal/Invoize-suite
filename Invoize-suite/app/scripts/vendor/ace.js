...
ace.handle_side_menu = function($) {
    ...
    $('#sidebar-collapse').on(ace.click_event, function() {
        ...
        if($minimized)
        {
            $('div#main-container').css('margin-top','0px');
            $('div.row.in-wiz-ttl.in-fix-scroll').css('margin-top','0px');
        }
        else
        {
            $('div#main-container').css('margin-top','-35px');
            $('div.row.in-wiz-ttl.in-fix-scroll').css('margin-top','-35px');
        }
...
$(document).ready(function(){
    ...
    var wW = $(window).width();
    if(wW < 1150)
        $('.navbar .pull-left .nav').addClass('nav-mini');
    else
        $('.navbar .pull-left .nav').removeClass('nav-mini');
