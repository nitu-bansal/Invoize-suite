...
angularApp.service('metadataService', function($http, $q) {
	return {
		...
		selectMetadataTypeEdit: function(data) {
			var deferred = $q.defer();
			$http({
				method: 'POST',
				url: '/api/read/metadataType',
				data: data
			}).
			success(function(data, status, headers, config) {
				if (data.type === 1) {
		...
		updateMetadataType: function(metadataType) {
			var deferred = $q.defer();
			$http({
				method: 'POST',
				url: '/api/update/metadataType',
				data: metadataType
			}).
			success(function(data, status, headers, config) {
				if (data.type === 1) {
