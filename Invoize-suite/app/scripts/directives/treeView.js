...
angularApp.directive('treeView', function() {
	...
	return {
		...
		priority: 1000,
		terminal: true,
		compile: function(tElement, tAttrs, transclude, scope) {
				...
				scope.$watch(rootExpr, function(root) {
					...
					(function walk(children, parentNode, parentScope, depth) {
						...
						for (; i < n; ++i) {
							...
							var desc;
							desc = "";
							angular.forEach(child.system, function(item) {
								desc += '<div class="pop-line"><i class="fa fa-desktop"></i> ' + item + '</div>';
							});
