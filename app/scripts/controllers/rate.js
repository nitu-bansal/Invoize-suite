...
angularApp.controller('rateCtrl', function($scope,$http,$location,$stateParams,$state,$routeParams,$route,rateService,flash){
       ...
       // $scope.otherFields = [{dn:"Zip Code",n:"zipCode"},{dn:"Origin Airport Code",n:"originAirportCode"},{dn:"Destination Airport Code",n:"destinationAirportCode"}];
        ...
        // chargeType contains list of charge types
        ...
        $scope.weightBracketFields = [{sph:"0",eph:"*"}];
                ...
                if($scope.weightBracketFields[index][field])
                {
                        if(field === "e" && $scope.weightBracketFields.length === index+1)
                        {
                                if($scope.weightBracketFields[index].e != "*")
                                {
                                        ...
                                        $scope.weightBracketFields.push({sph:start,eph:end+" or *"});
                                }
                        }
                }
                else
        ...
        $scope.searchCharge = function(value,accountId,templateId){               
               ...
               //console.log(localStorage.searchBarvals);
                ...
                var promise = rateService.chargeList({"accountId":accountId,"chargeDefnId":templateId,selected:value,pageLimit:10,page:1});
                promise.then(function(result){
        ...
        $scope.loadMore = function(accountId,templateId) {
                $scope.loadingRates = true;
                var promise = rateService.chargeList({"accountId":accountId,"chargeDefnId":templateId,selected:$scope.searchBarValue,pageLimit:10,page:counter});
                promise.then(function(result){
                                counter += 1;
                                $scope.loadingRates = false;
        ...
        $scope.create = function(charge,weightBracket,selectedChargeType,selectedRateQualifier,accountId,chargeDefnId,comment){
                var promise = rateService.create({"selection":charge , "weightBracket" : weightBracket,"chargeType":selectedChargeType,"rateQualifier":selectedRateQualifier,"accountId":accountId,"chargeDefnId":chargeDefnId,"comment":comment});
        ...
        $scope.createChargeDefn = function(chargeDefn,accountId){
                var promise = rateService.createChargeDefinition({"chargeDefn":chargeDefn , "accountId" : accountId});
                promise.then(function(msg){
        ...
        $scope.getAccountCodeList = function(q){      
                ...
                var promise = rateService.list({suggestionFor:'account', q:q, pageLimit:20,page:1});
                promise.then(function(result){
        ...
        $scope.loadMoreAccounts = function() {
                if(moreAccounts){
                        ...
                        var promise = rateService.list({suggestionFor:'account', q:'', pageLimit:20, page:accountCounter});
                        promise.then(function(result){
                                        if (result.length === 0)
                                                moreAccounts = false;
                                        accountCounter += 1;
        ...
        $scope.getOtherFieldsList = function(){
                var promise = rateService.otherFieldsList({suggestionFor:'user', q:'', pageLimit:10,page:1});
        ...
        $scope.getLaneParameter = function(){
                var promise = rateService.laneParameter({suggestionFor:'selectionCriteria', q:'', pageLimit:10,page:1});
                promise.then(function(r){       
        ...
        $scope.getChargeDefnGroup = function(){ //need to update suggestion for when API is ready
                var promise = rateService.chargeDefnGroups({suggestionFor:'selectionCriteria', q:'', pageLimit:10,page:1});
                promise.then(function(r){   
        ...
        $scope.getChargeDefnEffctBase = function(){ //need to update suggestion for when API is ready
                var promise = rateService.chargeDefneffectiveBase({suggestionFor:'selectionCriteria', q:'', pageLimit:10,page:1});
                promise.then(function(r){   
